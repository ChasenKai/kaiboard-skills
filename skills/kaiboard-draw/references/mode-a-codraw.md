...
- **`NO_BACKEND`**（`no available backend for storage=…`）：MCP 进程**既没起中继、也没绑文件夹** → 没有任何可用后端。多半是**启动参数漏了 `--relay`**；另一种情形是你**又起了一个中继进程** —— 那个进程因端口被占（`EADDRINUSE`）起不来，于是它自己**没有后端**（真正在服务的是先起的那个）。→ 核对启动参数，别重复起中继。
