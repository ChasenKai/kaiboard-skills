...
## [0.3.0] - 2026-09-21 · 文件树管理闭环（14 → 19 个命令）

跟进 `kaiboard-mcp` `0.4.0`：补齐文件树的建 / 改 / 移 / 排 / 删 / **还原**。
Follows `kaiboard-mcp` `0.4.0`: the full file-tree management loop.

### Added / 新增

- 命令表补齐至 **19 个**：新增 `createFolder` / `renameBoard` / `renameFolder` / `deleteFolder` / `listTrash` / `restoreNode` / `moveNode` / `reorderNode`。
- **回收站可还原** —— 删错了能自己恢复，而不只是"能删"。
- **结构可整理** —— `moveNode` + `reorderNode` 让你要求时可以**重排并重新归类整个画板树**。
